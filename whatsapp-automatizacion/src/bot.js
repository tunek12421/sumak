...
const DEPARTAMENTOS = {
    SUMAK: {
        nombre: "SUMAK - Gestión de Infraestructura",
        numero: "591XXXXXXXX", // Reemplazar con número real
        palabrasClave: ["inundación", "inundacion", "taponamiento", "alcantarillado", "alcantarilla", "agua", "drenaje", "obstrucción", "calle inundada"],
        descripcion: "Inundaciones, taponamiento de alcantarillado, gestión de agua"
    },
    UGR: {
        nombre: "UGR - Gestión de Riesgos",
        numero: "591XXXXXXXX", // Reemplazar con número real
        palabrasClave: ["desastre", "emergencia", "crisis", "damnificado", "colapso", "evacuación", "ayuda humanitaria", "catástrofe"],
        descripcion: "Desastres causados por inundación, gestión de emergencias"
    }
};

// Prompt del sistema mejorado para clasificación
const SYSTEM_PROMPT = `Eres un asistente de clasificación para el sistema municipal de gestión de emergencias de Cochabamba. Tu tarea es analizar reportes ciudadanos y determinar a qué departamento deben ser dirigidos.

Departamentos disponibles:
- SUMAK: Problemas de infraestructura como inundaciones, taponamiento de alcantarillado, drenaje, obstrucciones en alcantarillas, calles inundadas, problemas de agua en vías públicas
- UGR: Desastres y emergencias causados por inundaciones, situaciones de crisis, evacuaciones, personas damnificadas, colapsos estructurales, ayuda humanitaria, catástrofes
...
- Si el reporte menciona inundación pero sin daños graves o emergencia -> SUMAK
- Si el reporte menciona desastre, emergencia, damnificados o situación crítica por inundación -> UGR
...
"Buenos días" -> GENERAL
"😊" -> GENERAL
"La calle está inundada" -> SUMAK
"Alcantarilla tapada en mi calle" -> SUMAK
"Mi casa está inundada y necesito evacuación urgente" -> UGR
"Hay familias damnificadas por la inundación" -> UGR
"Colapso de puente por las lluvias" -> UGR`;
...
function generarMensajeRedireccion(departamento, mensajeOriginal, studentData = null) {
    ...
    if (!DEPARTAMENTOS[departamento]) {
        const mensajesGenerales = [
            `${saludo} Gracias por contactarnos. ¿Qué tipo de problema necesitas reportar?

Puedo ayudarte con:
• 💧 Inundaciones y alcantarillado → SUMAK
• 🚨 Emergencias y desastres → UGR

¿Cuál es tu situación?`,

            `${saludo} ¡Bienvenido/a al sistema de emergencias municipal!

Por favor, indícame tu situación:
• Inundación o alcantarilla tapada → SUMAK
• Emergencia o desastre → UGR
